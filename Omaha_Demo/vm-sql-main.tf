...
resource "azurerm_network_interface" "terraform-sql-server-interface" {
  name                = "terraform-sql-server-interface"
...
resource "azurerm_windows_virtual_machine" "sql" {
  ...
  network_interface_ids = [
    azurerm_network_interface.terraform-sql-server-interface.id,
  ]
...
resource "azurerm_dev_test_global_vm_shutdown_schedule" "sql-shutdown" {
