...
resource "azurerm_network_security_rule" "udp_53_client_outbound" {
  ...
  network_security_group_name = azurerm_network_security_group.active-directory-client-nsg.name
  resource_group_name         = azurerm_resource_group.terraform-resource-group.name
  name                        = "AD_53_DNS_UDP-DC${count.index+1} Outbound"
  description                 = "AD_53_DNS_UDP-DC${count.index+1} Outbound"
  priority                    = (150 + count.index)
  direction                   = "Outbound"
  access                      = "Allow"
  protocol                    = "Udp"
...
resource "azurerm_network_security_rule" "tcp_88_client_outbound" {
  ...
  network_security_group_name = azurerm_network_security_group.active-directory-client-nsg.name
  resource_group_name         = azurerm_resource_group.terraform-resource-group.name
  name                        = "AD_88_Kerberos_TCP-DC${count.index+1} Outbound"
  description                 = "AD_88_Kerberos_TCP-DC${count.index+1} Outbound"
  priority                    = (160 + count.index)
  direction                   = "Outbound"
  access                      = "Allow"
  protocol                    = "Tcp"
...
resource "azurerm_network_security_rule" "tcp_135_client_outbound" {
  ...
  network_security_group_name = azurerm_network_security_group.active-directory-client-nsg.name
  resource_group_name         = azurerm_resource_group.terraform-resource-group.name
  name                        = "AD_135_RPC_TCP-DC${count.index+1} Outbound"
  description                 = "AD_135_RPC_TCP-DC${count.index+1} Outbound"
  priority                    = (170 + count.index)
  direction                   = "Outbound"
  access                      = "Allow"
  protocol                    = "Tcp"
...
resource "azurerm_network_security_rule" "tcp_389_client_outbound" {
  ...
  network_security_group_name = azurerm_network_security_group.active-directory-client-nsg.name
  resource_group_name         = azurerm_resource_group.terraform-resource-group.name
  name                        = "AD_389_LDAP_TCP-DC${count.index+1} Outbound"
  description                 = "AD_389_LDAP_TCP-DC${count.index+1} Outbound"
  priority                    = (180 + count.index)
  direction                   = "Outbound"
  access                      = "Allow"
  protocol                    = "Tcp"
...
resource "azurerm_network_security_rule" "tcp_445_client_outbound" {
  ...
  network_security_group_name = azurerm_network_security_group.active-directory-client-nsg.name
  resource_group_name         = azurerm_resource_group.terraform-resource-group.name
  name                        = "AD_445_SMB_TCP-DC${count.index+1} Outbound"
  description                 = "AD_445_SMB_TCP-DC${count.index+1} Outbound"
  priority                    = (190 + count.index)
  direction                   = "Outbound"
  access                      = "Allow"
  protocol                    = "Tcp"
...
resource "azurerm_network_security_rule" "tcp_49152-65535_client_outbound" {
  ...
  network_security_group_name = azurerm_network_security_group.active-directory-client-nsg.name
  resource_group_name         = azurerm_resource_group.terraform-resource-group.name
  name                        = "AD_49152-65535_TCP-DC${count.index+1} Outbound"
  description                 = "AD_49152-65535_TCP-DC${count.index+1} Outbound"
  priority                    = (200 + count.index)
  direction                   = "Outbound"
  access                      = "Allow"
  protocol                    = "Tcp"
...
resource "azurerm_network_security_rule" "udp_49152-65535_client_outbound" {
  ...
  network_security_group_name = azurerm_network_security_group.active-directory-client-nsg.name
  resource_group_name         = azurerm_resource_group.terraform-resource-group.name
  name                        = "AD_49152-65535_UDP-DC${count.index+1} Outbound"
  description                 = "AD_49152-65535_UDP-DC${count.index+1} Outbound"
  priority                    = (210 + count.index)
  direction                   = "Outbound"
  access                      = "Allow"
  protocol                    = "Udp"
...
resource "azurerm_network_security_rule" "icmp_client_outbound" {
  ...
  network_security_group_name = azurerm_network_security_group.active-directory-client-nsg.name
  resource_group_name         = azurerm_resource_group.terraform-resource-group.name
  name                        = "AD_PingtoDC${count.index+1} Outbound"
  description                 = "AD_PingtoDC${count.index+1} Outbound"
  priority                    = (220 + count.index)
  direction                   = "Outbound"
  access                      = "Allow"
  protocol                    = "Icmp"
