...
resource "azurerm_network_interface" "terraform-ubuntu-web2-interface" {
  name                = "terraform-ubuntu-web2-interface"
...
resource "azurerm_linux_virtual_machine" "terraform-ubuntu-web2-machine" {
  name                = "terraform-ubuntu-web2-machine"
  resource_group_name = azurerm_resource_group.terraform-resource-group.name
  location            = var.location
  size                = var.ubuntu_vm_size
  admin_username      = var.ubuntu_admin_user
  network_interface_ids = [
    azurerm_network_interface.terraform-ubuntu-web2-interface.id
  ]
  admin_ssh_key {
...
resource "azurerm_dev_test_global_vm_shutdown_schedule" "web2" {
    virtual_machine_id = azurerm_linux_virtual_machine.terraform-ubuntu-web2-machine.id
